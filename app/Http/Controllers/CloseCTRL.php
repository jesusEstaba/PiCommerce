...
class CloseCTRL extends Controller
{
    ...
    public static function hora()
    {
        ...
        if(!$days->closed)
        {
            ...
            else if($dia=="Tuesday" && $hora>=$days->tue_open && $hora<=$days->tue_close )
                return true;

            else if($dia=="Wednesday" && $hora>=$days->wed_open && $hora<=$days->wed_close )
                return true;

            else if($dia=="Thursday" && $hora>=$days->thu_open && $hora<=$days->thu_close )
                return true;
            
            else if($dia=="Friday" && $hora>=$days->fri_open && $hora<=$days->fri_close )
                return true;

            else if($dia=="Saturday" && $hora>=$days->sat_open && $hora<=$days->sat_close )
                return true;

            else if($dia=="Sunday" && $hora>=$days->sun_open && $hora<=$days->sun_close )
                return true;
        }
